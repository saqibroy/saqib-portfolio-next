...
export default function WorkPage() {
  return <div className="work-page">
    ...
    <section className="work-grid" aria-label="Case studies">{caseStudies.map((item) => <article key={item.slug} className="work-card">
      ...
      <p className="work-card-result"><strong>Result:</strong> {item.result}</p><ul aria-label={`${item.title} stack`}>{item.stack.slice(0, 5).map((value) => <li key={value}>{value}</li>)}</ul><Link href={`/work/${item.slug}`}>Read case study →</Link>
    </article>)}</section>
  </div>;
}
