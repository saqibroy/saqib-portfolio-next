...
const labels: Record<Variant, string[]> = {
  ai: ['Django data', 'React chatbot', 'FastAPI AI', 'Editing session', 'Generated contract', 'Contract configurator'],
...
export function ProjectSystemVisual({ variant, showCaption = false, size = 'compact' }: { variant: Variant; showCaption?: boolean; size?: 'compact' | 'expanded' }) {
  ...
  const caption = variant === 'ai' ? 'The chatbot creates or updates Django editing sessions before the generated contract opens in the configurator.' : variant === 'content' ? 'Shared content delivery and search integration, based on the Phase 13 repository audit.' : variant === 'jobs' ? 'Accepted roles continue to alerts; non-matching roles exit before notification.' : 'Authenticated dashboard actions move through the API to an explicit crawl lifecycle and stored state.';
